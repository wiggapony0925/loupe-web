...
// ── Carousel registry (dev portal control room) ─────────────────────────

/** One carousel recipe as stored in the registry / authored by AI. */
export interface CarouselRecipeDef {
  id: string;
  title: string;
  subtitle: string;
...
/** Registry entry annotated for the portal (origin + live-edit badges). */
export interface AdminCarouselRecipe extends CarouselRecipeDef {
  enabled: boolean;
  /** null = every priced game; explicit list scopes verbatim. */
  games: string[] | null;
  origin: "file" | "custom";
  edited: boolean;
  removed: boolean;
...
/** Everything /admin/carousels renders in one call. */
export interface AdminCarouselsView {
  aiConfigured: boolean;
  aiEnabled: boolean;
  recipes: AdminCarouselRecipe[];
  games: AdminCarouselGame[];
  ai: Record<string, CarouselRecipeDef[]>;
}

/** Partial recipe edit — only sent fields are patched. */
export interface CarouselRecipeUpdate {
  enabled?: boolean;
  title?: string;
  subtitle?: string;
  source?: "value" | "trending" | "catalog";
...
/** Operator-authored new recipe. */
export interface CarouselRecipeCreate extends Partial<CarouselRecipeDef> {
  id: string;
  title: string;
  subtitle: string;
  enabled?: boolean;
