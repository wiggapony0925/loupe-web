...
export const useTrending = () => useApiQuery<CardSummary[]>(["trending"], api.cards.trending, { staleTime: 60_000 });
...
export const usePublicSearch = (params: PublicSearchParams, enabled = true) =>
  useApiQuery<SearchPage>(["public-search", params], () => api.cards.publicSearch(params), {
    enabled,
    staleTime: 60_000,
...
export const usePublicBrowse = (params: PublicBrowseParams, enabled = true) =>
  useApiQuery<SearchPage>(["public-browse", params], () => api.cards.publicBrowse(params), {
    enabled,
    staleTime: 60_000,
...
export const usePublicTrending = (params: PublicTrendingParams = {}) =>
  useApiQuery<CardSummary[]>(["public-trending", params], () => api.cards.publicTrending(params), {
    staleTime: 60_000,
  });
...
export const useMarket = (id: string) =>
  useApiQuery<CardMarket>(["market", id], () => api.cards.market(id), { enabled: Boolean(id), staleTime: 300_000 });

/** Live per-marketplace prices + buy links. */
export const useMarketplacePrices = (id: string) =>
  useApiQuery<MarketplaceQuote[]>(["marketplace-prices", id], () => api.cards.marketplacePrices(id), {
    enabled: Boolean(id),
    staleTime: 120_000,
  });
...
export const useCard = (id: string) =>
  useApiQuery<CardSummary>(["card", id], () => api.cards.canonical(id), { enabled: Boolean(id), staleTime: 300_000 });

/** Current user — enable once a token is present. */
export const useMe = (enabled: boolean) => useApiQuery<User>(["me"], api.me.get, { enabled, retry: false });

/** Sign-in mutation. */
export const useLogin = () => useApiMutation<TokenPair, SignInRequest>(api.auth.login);

/** Sign-up mutation. */
export const useRegister = () => useApiMutation<TokenPair, SignUpRequest>(api.auth.register);

/** Whole-portfolio analytics (stats, set indexes, movers, distributions). */
export const useAnalyticsOverview = (enabled = true) =>
  useApiQuery<AnalyticsOverview>(["analytics-overview"], api.analytics.overview, { enabled, staleTime: 60_000 });

/** Authenticated home feed — top movers + recent scans. */
export const useHomeFeed = (
  params?: { topMovers?: number; recentScans?: number },
  enabled = true,
) => useApiQuery<HomeFeed>(["home-feed", params], () => api.home.feed(params), { enabled, staleTime: 60_000 });

/** The signed-in user's watchlist. */
export const useWatchlist = (enabled = true) =>
  useApiQuery<WatchlistItem[]>(["watchlist"], api.watchlist.list, { enabled, staleTime: 30_000 });

/** The signed-in user's graded/owned cards (the Vault). */
export const useGrades = (params?: GradesParams, enabled = true) =>
  useApiQuery<GradedCard[]>(["grades", params], () => api.grades.list(params), { enabled, staleTime: 30_000 });

/** Pin a public card (composite id) to the watchlist — resolves then adds. */
export const useAddToWatchlist = (
  options?: Omit<UseMutationOptions<WatchlistItem, ApiError, string>, "mutationFn">,
) => useApiMutation<WatchlistItem, string>((upstreamId) => api.watchlist.addByUpstream(upstreamId), options);
...
export const useResolvedCardId = (upstreamId: string, enabled = true) =>
  useApiQuery<string | null>(["resolve", upstreamId], () => api.cards.resolve(upstreamId), {
...
export const useAddGrade = (
  options?: Omit<UseMutationOptions<GradedCard, ApiError, CreateGradeInput>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<GradedCard, CreateGradeInput>((input) => api.grades.create(input), {
    ...options,
    onSuccess: (...args) => {
...
export const useUpdateGrade = (
  options?: Omit<UseMutationOptions<GradedCard, ApiError, UpdateGradeInput>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<GradedCard, UpdateGradeInput>((input) => api.grades.update(input), {
    ...options,
    onSuccess: (...args) => {
      invalidateGradeCaches(qc);
      options?.onSuccess?.(...args);
...
export const useJobs = () => useApiQuery<JobPosting[]>(["jobs"], api.careers.jobs, { staleTime: 60_000 });

/** A single open role by slug. */
export const useJob = (slug: string) =>
  useApiQuery<JobPosting>(["job", slug], () => api.careers.job(slug), { enabled: Boolean(slug) });

/** Submit an application to an open role. */
export const useApplyToJob = (
  options?: Omit<UseMutationOptions<ApplicationSubmitted, ApiError, { jobId: string; input: ApplyInput }>, "mutationFn">,
...
export const useTrackApplication = (applicationId: string, email: string, enabled: boolean) =>
  useApiQuery<ApplicationTrack>(
    ["application-track", applicationId, email],
    () => api.careers.track(applicationId, email),
    { enabled: enabled && Boolean(applicationId) && Boolean(email), retry: false },
  );

/** Published blog posts. */
export const useBlogPosts = () => useApiQuery<BlogPost[]>(["blog-posts"], api.blog.posts, { staleTime: 60_000 });

/** A single published post by slug. */
export const useBlogPost = (slug: string) =>
  useApiQuery<BlogPost>(["blog-post", slug], () => api.blog.post(slug), { enabled: Boolean(slug) });
...
export const useWaitlistStats = (enabled = true) =>
  useApiQuery<WaitlistStats>(["waitlist-stats"], api.waitlist.stats, { enabled, staleTime: 60_000 });

/** Join the scanner waitlist (the checkout CTA); refreshes stats. */
export const useJoinWaitlist = (
  options?: Omit<UseMutationOptions<WaitlistJoined, ApiError, WaitlistJoinInput>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<WaitlistJoined, WaitlistJoinInput>((input) => api.waitlist.join(input), {
    ...options,
    onSuccess: (...args) => {
...
export const useCreateJob = (
  options?: Omit<UseMutationOptions<JobPosting, ApiError, JobPostingInput>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<JobPosting, JobPostingInput>((input) => api.admin.jobs.create(input), {
    ...options,
    onSuccess: (...args) => {
...
export const useUpdateJob = (
  options?: Omit<UseMutationOptions<JobPosting, ApiError, { id: string; input: JobPostingInput }>, "mutationFn">,
) => {
  const qc = useQueryClient();
...
export const useAdminApplications = (params?: { jobId?: string; status?: string }, enabled = true) =>
  useApiQuery<JobApplication[]>(["admin-applications", params ?? {}], () => api.admin.applications.list(params), {
    enabled,
    staleTime: 15_000,
  });
...
export const useAdminApplication = (id: string, enabled = true) =>
  useApiQuery<JobApplicationDetail>(["admin-application", id], () => api.admin.applications.get(id), {
...
export const useUpdateApplicationStatus = (
  options?: Omit<
    UseMutationOptions<JobApplicationDetail, ApiError, { id: string; input: ApplicationStatusUpdateInput }>,
    "mutationFn"
  >,
) => {
  const qc = useQueryClient();
  return useApiMutation<JobApplicationDetail, { id: string; input: ApplicationStatusUpdateInput }>(
...
export const useCreateBlogPost = (
  options?: Omit<UseMutationOptions<BlogPost, ApiError, BlogPostInput>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<BlogPost, BlogPostInput>((input) => api.admin.blog.create(input), {
    ...options,
    onSuccess: (...args) => {
...
export const useUpdateBlogPost = (
  options?: Omit<UseMutationOptions<BlogPost, ApiError, { id: string; input: BlogPostInput }>, "mutationFn">,
) => {
  const qc = useQueryClient();
...
export const useAdminWaitlist = (params?: { status?: WaitlistStatus }, enabled = true) =>
  useApiQuery<WaitlistEntry[]>(["admin-waitlist", params ?? {}], () => api.admin.waitlist.list(params), {
    enabled,
    staleTime: 15_000,
  });
...
export const useSetWaitlistStatus = (
  options?: Omit<UseMutationOptions<WaitlistEntry, ApiError, { id: string; status: WaitlistStatus }>, "mutationFn">,
) => {
  const qc = useQueryClient();
...
export const useAdminMetrics = (enabled = true) =>
  useApiQuery<AdminMetrics>(["admin-metrics"], api.admin.metrics, { enabled, staleTime: 30_000 });

/** Searchable, paginated user list. */
export const useAdminUsers = (params?: AdminUsersParams, enabled = true) =>
  useApiQuery<AdminUserPage>(["admin-users", params ?? {}], () => api.admin.users.list(params), {
    enabled,
    staleTime: 15_000,
    placeholderData: (prev) => prev,
  });
...
export const useSetUserRole = (
  options?: Omit<UseMutationOptions<AdminUser, ApiError, { id: string; isAdmin: boolean }>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<AdminUser, { id: string; isAdmin: boolean }>(
    ({ id, isAdmin }) => api.admin.users.setRole(id, isAdmin),
    { ...options, onSuccess: (...a) => { invalidateUsers(qc); options?.onSuccess?.(...a); } },
  );
};

/** Ban a user (optionally with a reason). */
export const useBanUser = (
  options?: Omit<UseMutationOptions<AdminUser, ApiError, { id: string; reason?: string | null }>, "mutationFn">,
) => {
  const qc = useQueryClient();
...
export const useUnbanUser = (
  ...
  return useApiMutation<AdminUser, string>((id) => api.admin.users.unban(id), {
    ...options,
    onSuccess: (...a) => { invalidateUsers(qc); options?.onSuccess?.(...a); },
  });
};
...
export const useAdminUserDetail = (id: string, enabled = true) =>
  useApiQuery<AdminUserDetail>(["admin-user", id], () => api.admin.users.get(id), {
    enabled: enabled && Boolean(id),
  });
...
export const useAdminFlags = (enabled = true) =>
  useApiQuery<FeatureFlag[]>(["admin-flags"], api.admin.flags.list, { enabled });
...
export const useCreateFlag = (
  options?: Omit<UseMutationOptions<FeatureFlag, ApiError, FeatureFlagCreateInput>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<FeatureFlag, FeatureFlagCreateInput>((input) => api.admin.flags.create(input), {
    ...options,
    onSuccess: (...a) => { invalidateFlags(qc); options?.onSuccess?.(...a); },
  });
};

/** Update / toggle a feature flag. */
export const useUpdateFlag = (
  options?: Omit<UseMutationOptions<FeatureFlag, ApiError, { id: string; input: FeatureFlagUpdateInput }>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<FeatureFlag, { id: string; input: FeatureFlagUpdateInput }>(
    ({ id, input }) => api.admin.flags.update(id, input),
    { ...options, onSuccess: (...a) => { invalidateFlags(qc); options?.onSuccess?.(...a); } },
  );
};
...
export const useUpsertFlag = (
  options?: Omit<UseMutationOptions<FeatureFlag, ApiError, { key: string; input: FeatureFlagUpsertInput }>, "mutationFn">,
) => {
  const qc = useQueryClient();
  return useApiMutation<FeatureFlag, { key: string; input: FeatureFlagUpsertInput }>(
