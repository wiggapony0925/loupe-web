...
export const api = {
  cards: {
    ...
    search: async (q: string): Promise<CardSummary[]> => {
      const d = await apiFetch<{ results: ApiCard[] }>(ENDPOINTS.cards.search, { query: { q } });
    ...
    publicSearch: async (params: PublicSearchParams): Promise<SearchPage> => {
      ...
      return {
        ...
        facets: { rarities: d.facets?.rarities ?? [], sets: d.facets?.sets ?? [] },
      };
    },
    ...
    publicBrowse: async (params: PublicBrowseParams): Promise<SearchPage> => {
      const d = await apiFetch<{ cards: ApiCard[]; total: number; page: number; page_size: number }>(
    ...
    publicTrending: async (params: PublicTrendingParams = {}): Promise<CardSummary[]> => {
      const d = await apiFetch<{ cards: ApiCard[] }>(ENDPOINTS.public.trending, {
        query: {
          tcg: params.tcg ?? "all",
    ...
    market: async (id: string): Promise<CardMarket> => {
      const d = await apiFetch<{ snapshot?: { summary?: Record<string, never> } }>(ENDPOINTS.cards.market(id));
    ...
    canonical: async (id: string): Promise<CardSummary> => {
      const d = await apiFetch<Parameters<typeof canonicalToSummary>[1]>(ENDPOINTS.cards.canonical(id));
    ...
    resolve: async (upstreamId: string): Promise<string | null> => {
      const d = await apiFetch<{ card_id: string | null }>(ENDPOINTS.cards.resolve, {
        method: "POST",
  ...
  auth: {
    ...
    login: (body: SignInRequest) => apiFetch<TokenPair>(ENDPOINTS.auth.login, { method: "POST", json: body, skipAuth: true }),
    /** Email + password sign-up → TokenPair. */
    register: (body: SignUpRequest) =>
      apiFetch<TokenPair>(ENDPOINTS.auth.register, { method: "POST", json: body, skipAuth: true }),
    /** Passwordless dev login (email only) → TokenPair. */
    devLogin: (body: { email: string; display_name?: string }) =>
      apiFetch<TokenPair>(ENDPOINTS.auth.devLogin, { method: "POST", json: body, skipAuth: true }),
  },
  me: {
  ...
  home: {
    ...
    feed: (params?: { topMovers?: number; recentScans?: number }) =>
      apiFetch<HomeFeed>(ENDPOINTS.home.feed, {
        query: { topMovers: params?.topMovers, recentScans: params?.recentScans },
      }),
  },
  watchlist: {
    ...
    /** Unpin by local UUID. */
    remove: (cardId: string) => apiFetch<void>(ENDPOINTS.watchlist.remove(cardId), { method: "DELETE" }),
    /**
  ...
  grades: {
    ...
    create: async (input: CreateGradeInput): Promise<GradedCard> => {
      const body: Record<string, unknown> = { grade: input.grade, house: input.house };
      ...
      if (input.purchasePriceUsd != null) body.purchase_price_usd = input.purchasePriceUsd;
      if (input.purchaseDate) body.purchase_date = input.purchaseDate;
      if (input.estimatedValueUsd != null) body.estimated_value_usd = input.estimatedValueUsd;
      if (input.notes) body.notes = input.notes;
      const row = await apiFetch<RawGrade>(ENDPOINTS.grades.create, { method: "POST", json: body });
    ...
    update: async ({ id, ...rest }: UpdateGradeInput): Promise<GradedCard> => {
      ...
      if (rest.purchasePriceUsd !== undefined) body.purchase_price_usd = rest.purchasePriceUsd;
      if (rest.purchaseDate !== undefined) body.purchase_date = rest.purchaseDate;
      if (rest.estimatedValueUsd !== undefined) body.estimated_value_usd = rest.estimatedValueUsd;
      if (rest.notes !== undefined) body.notes = rest.notes;
      const row = await apiFetch<RawGrade>(ENDPOINTS.grades.item(id), { method: "PATCH", json: body });
      return toGradedCard(row);
    },
    /** Soft-delete a holding. The card stays in the catalog. */
    remove: (id: string) => apiFetch<void>(ENDPOINTS.grades.item(id), { method: "DELETE" }),
  },
  /** Public careers surface — browse open roles, apply, track. */
  careers: {
    jobs: async (): Promise<JobPosting[]> => {
      const rows = await apiFetch<Parameters<typeof toJobPosting>[0][]>(ENDPOINTS.careers.jobs, { skipAuth: true });
      return (rows ?? []).map(toJobPosting);
    },
    job: async (slug: string): Promise<JobPosting> =>
      toJobPosting(await apiFetch(ENDPOINTS.careers.job(slug), { skipAuth: true })),
    apply: async (jobId: string, input: ApplyInput): Promise<ApplicationSubmitted> =>
      toApplicationSubmitted(
        await apiFetch(ENDPOINTS.careers.apply(jobId), { method: "POST", json: applyToBody(input), skipAuth: true }),
      ),
    track: async (applicationId: string, email: string): Promise<ApplicationTrack> =>
      toApplicationTrack(
        await apiFetch(ENDPOINTS.careers.track(applicationId), { query: { email }, skipAuth: true }),
      ),
  },
  /** Public blog surface — published posts only. */
  blog: {
    posts: async (): Promise<BlogPost[]> => {
      const rows = await apiFetch<Parameters<typeof toBlogPost>[0][]>(ENDPOINTS.blog.posts, { skipAuth: true });
  ...
  waitlist: {
    ...
    join: async (input: WaitlistJoinInput): Promise<WaitlistJoined> =>
      toWaitlistJoined(await apiFetch(ENDPOINTS.waitlist.join, { method: "POST", json: waitlistJoinToBody(input) })),
    stats: async (): Promise<WaitlistStats> =>
      toWaitlistStats(await apiFetch(ENDPOINTS.waitlist.stats, { skipAuth: true })),
  },
  /** Public feature-flag map — `{key: enabled}`. No auth (web + mobile gate on it). */
  flags: async (): Promise<FlagMap> => (await apiFetch<FlagMap>(ENDPOINTS.flags, { skipAuth: true })) ?? {},
  /** Admin developer-portal surface (requires an admin user). */
  admin: {
    /** At-a-glance portal metrics. */
    metrics: async (): Promise<AdminMetrics> => toAdminMetrics(await apiFetch(ENDPOINTS.admin.metrics)),
    users: {
      ...
      get: async (id: string): Promise<AdminUserDetail> => toAdminUserDetail(await apiFetch(ENDPOINTS.admin.user(id))),
      createTest: async (): Promise<TestAccount> => apiFetch<TestAccount>(ENDPOINTS.admin.userTest, { method: "POST" }),
      setRole: async (id: string, isAdmin: boolean): Promise<AdminUser> =>
        toAdminUser(await apiFetch(ENDPOINTS.admin.userRole(id), { method: "PATCH", json: { is_admin: isAdmin } })),
      ban: async (id: string, reason?: string | null): Promise<AdminUser> =>
        toAdminUser(await apiFetch(ENDPOINTS.admin.userBan(id), { method: "POST", json: { reason: reason ?? null } })),
      unban: async (id: string): Promise<AdminUser> =>
        toAdminUser(await apiFetch(ENDPOINTS.admin.userUnban(id), { method: "POST" })),
      remove: (id: string) => apiFetch<void>(ENDPOINTS.admin.user(id), { method: "DELETE" }),
    },
    flags: {
      list: async (): Promise<FeatureFlag[]> => {
        const rows = await apiFetch<Parameters<typeof toFeatureFlag>[0][]>(ENDPOINTS.admin.flags);
        return (rows ?? []).map(toFeatureFlag);
      },
      create: async (input: FeatureFlagCreateInput): Promise<FeatureFlag> =>
        toFeatureFlag(await apiFetch(ENDPOINTS.admin.flags, { method: "POST", json: input })),
      update: async (id: string, input: FeatureFlagUpdateInput): Promise<FeatureFlag> =>
        toFeatureFlag(await apiFetch(ENDPOINTS.admin.flag(id), { method: "PATCH", json: input })),
      /** Set a flag's enabled state by key, creating it if missing (inspect overlay). */
      upsertByKey: async (key: string, input: FeatureFlagUpsertInput): Promise<FeatureFlag> =>
        toFeatureFlag(await apiFetch(ENDPOINTS.admin.flagByKey(key), { method: "PUT", json: input })),
      remove: (id: string) => apiFetch<void>(ENDPOINTS.admin.flag(id), { method: "DELETE" }),
    },
    jobs: {
      list: async (): Promise<JobPosting[]> => {
        const rows = await apiFetch<Parameters<typeof toJobPosting>[0][]>(ENDPOINTS.admin.jobs);
        return (rows ?? []).map(toJobPosting);
      },
      create: async (input: JobPostingInput): Promise<JobPosting> =>
        toJobPosting(await apiFetch(ENDPOINTS.admin.jobs, { method: "POST", json: jobPostingToBody(input) })),
      update: async (id: string, input: JobPostingInput): Promise<JobPosting> =>
        toJobPosting(await apiFetch(ENDPOINTS.admin.job(id), { method: "PATCH", json: jobPostingToBody(input) })),
      remove: (id: string) => apiFetch<void>(ENDPOINTS.admin.job(id), { method: "DELETE" }),
    },
    applications: {
      list: async (params?: { jobId?: string; status?: string }): Promise<JobApplication[]> => {
        const rows = await apiFetch<Parameters<typeof toJobApplication>[0][]>(ENDPOINTS.admin.applications, {
      ...
      updateStatus: async (id: string, input: ApplicationStatusUpdateInput): Promise<JobApplicationDetail> =>
        toJobApplicationDetail(
          await apiFetch(ENDPOINTS.admin.applicationStatus(id), { method: "PATCH", json: statusUpdateToBody(input) }),
        ),
    },
    blog: {
      list: async (): Promise<BlogPost[]> => {
        const rows = await apiFetch<Parameters<typeof toBlogPost>[0][]>(ENDPOINTS.admin.blog);
        return (rows ?? []).map(toBlogPost);
      },
      create: async (input: BlogPostInput): Promise<BlogPost> =>
        toBlogPost(await apiFetch(ENDPOINTS.admin.blog, { method: "POST", json: blogPostToBody(input) })),
      update: async (id: string, input: BlogPostInput): Promise<BlogPost> =>
        toBlogPost(await apiFetch(ENDPOINTS.admin.blogPost(id), { method: "PATCH", json: blogPostToBody(input) })),
      remove: (id: string) => apiFetch<void>(ENDPOINTS.admin.blogPost(id), { method: "DELETE" }),
    },
    waitlist: {
      list: async (params?: { status?: WaitlistStatus }): Promise<WaitlistEntry[]> => {
        const rows = await apiFetch<Parameters<typeof toWaitlistEntry>[0][]>(ENDPOINTS.admin.waitlist, {
          query: { status: params?.status },
        });
        return (rows ?? []).map(toWaitlistEntry);
      },
      setStatus: async (id: string, status: WaitlistStatus): Promise<WaitlistEntry> =>
        toWaitlistEntry(await apiFetch(ENDPOINTS.admin.waitlistStatus(id), { method: "PATCH", json: { status } })),
      remove: (id: string) => apiFetch<void>(ENDPOINTS.admin.waitlistEntry(id), { method: "DELETE" }),
    },
  },
} as const;
